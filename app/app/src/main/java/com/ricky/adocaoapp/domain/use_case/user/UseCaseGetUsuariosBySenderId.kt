...
class UseCaseGetUsuariosBySenderId @Inject constructor(private val repository: UserRepository) {

    operator fun invoke(idUser: String): Flow<Resource<List<Usuario>>> = flow {
        try {
            emit(Resource.Loading())

            repository.getUsuariosBySenderId(idUser).let { result ->
